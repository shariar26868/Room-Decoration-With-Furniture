...
# # =================================================================
# # STEP 4: SET ROOM DIMENSIONS (CENTIMETERS)
# # =================================================================
# @router.post("/dimensions")
# async def set_dimensions(request: RoomDimensionRequest):
#     """Set room dimensions in CENTIMETERS"""
#     session = get_session(request.session_id)
    
#     if not session.room_type or not session.theme:
#         raise HTTPException(status_code=400, detail="Complete previous steps first")
    
#     session.length = request.length  # in cm
#     session.width = request.width    # in cm
#     session.height = request.height  # in cm
#     session.square_feet = SpaceCalculator.calculate_room_area(request.length, request.width)  # sq cm
#     session.cubic_feet = SpaceCalculator.calculate_cubic_volume(request.length, request.width, request.height)  # cubic cm
    
#     logger.info(f"✅ Dimensions: {request.length} cm × {request.width} cm × {request.height} cm = {session.square_feet:.2f} sq cm")
...
#         "width": request.width,
#         "height": request.height,
#         "square_cm": round(session.square_feet, 2),
#         "cubic_cm": round(session.cubic_feet, 2),
#         "message": f"Room: {session.square_feet:.2f} sq cm"
#     }
...
#         req.furniture_type,
#         req.subtype,
#         session.square_feet  # room area in sq cm
#     )
...
#         "subtype": req.subtype,
#         "dimensions": dimensions,
#         "sqcm": dimensions["sqcm"]  # ✅ FIXED: Changed from 'sqft' to 'sqcm'
#     }
    
#     session.furniture_selections.append(furniture_item)
#     session.furniture_total_sqft = validation["total_sqcm"]  # Update with sq cm
    
#     logger.info(f"✅ Added: {req.subtype} ({dimensions['sqcm']:.2f} sq cm)")
...
#             temp_selections.append(furniture_item)
#             logger.info(f"   ✅ {dimensions['sqcm']:.2f} sq cm")
...
#     # Validate total space
#     if temp_selections:
#         total_new_sqcm = sum(item["sqcm"] for item in temp_selections)
#         current_sqcm = session.furniture_total_sqft
#         combined_total = current_sqcm + total_new_sqcm
        
#         room_sqcm = session.square_feet
#         usage_percent = (combined_total / room_sqcm) * 100
#         remaining_sqcm = room_sqcm - combined_total
        
#         from ai_backend.config import MAX_ROOM_USAGE_PERCENT
        
#         logger.info(f"\n📊 Space: {combined_total:.2f} sq cm ({usage_percent:.1f}%)")
...
#     removed = session.furniture_selections.pop(index)
#     session.furniture_total_sqft = sum(item.get("sqcm", 0) for item in session.furniture_selections)  # ✅ Changed to sqcm
...
#         "furniture_items": session.furniture_selections,
#         "count": len(session.furniture_selections),
#         "total_sqcm": round(session.furniture_total_sqft, 2),
#         "room_sqcm": session.square_feet,
#         "usage_percent": round(usage_percent, 2),
#         "remaining_sqcm": round(session.square_feet - session.furniture_total_sqft, 2) if session.square_feet else 0,
#         "message": f"{len(session.furniture_selections)} items selected"
#     }
...
#             "width": session.width,
#             "height": session.height,
#             "square_cm": session.square_feet
#         } if session.square_feet else None,
#         "furniture": {
#             "items": session.furniture_selections,
#             "count": len(session.furniture_selections),
#             "total_sqcm": session.furniture_total_sqft,
#             "usage_percent": round((session.furniture_total_sqft / session.square_feet * 100), 2) if session.square_feet else 0
#         },
#         "price_range": {
#             "min_price": session.min_price,
...
async def select_furniture_bulk(req: BulkFurnitureSelectionRequest, request: Request):
    ...
    if temp_selections:
        total_new_sqcm = sum(item["sqcm"] for item in temp_selections)
        current_sqcm = getattr(session, 'furniture_total_sqft', 0.0)  # ✅ Safe access with default
        combined_total = current_sqcm + total_new_sqcm
...
async def remove_furniture(session_id: str, index: int):
    ...
    removed = session.furniture_selections.pop(index)
    session.furniture_total_sqft = sum(item.get("sqcm", 0) for item in session.furniture_selections)
